...
- **Restack** — **Restack onto `<base>`** 는 최신 부모 브랜치 위로 rebase 한 뒤
  ...
  해결하면 됩니다.
- **에이전트로 rebase 충돌 해결** — 버튼 하나로 해당 워크스페이스의 에이전트에게 충돌을
  맡길 수 있습니다. 자동 시작은 기본으로 꺼져 있으며 Settings 에서 선택해 켤 수 있습니다.
...
- **자기 변경을 눈으로 확인하기** — `open_preview` 는 Wooi 의 프리뷰를 이 워크스페이스의 dev
  서버로 보내고, `capture_preview` 는 그 화면을 이미지로 돌려주며, `read_preview_issues` 는 그
  페이지의 콘솔 에러와 실패한 요청을 돌려줍니다. 에이전트가 조작하는 것은 사용자가 보고 있는 그
  패널이라, 에이전트가 무엇을 보고 있는지도 함께 보입니다. 읽기만 합니다 — 클릭·입력 도구는 없습니다.
- **일상적인 도구도** — `open_pull_request`(base 는 Wooi 가 고릅니다 — 스택이면 부모 브랜치,
  아니면 기본 브랜치), `list_issues`, 그리고 `run_script`·`stop_script`·`read_script_output`.
  마지막 셋은 리포에 설정해 둔 스크립트를 다룹니다.
- **자기 뒤를 치우기** — `archive_workspace` 는 끝난 워크스페이스를 접고, `delete_workspace` 는
  아예 지웁니다. 리포를 가리지 않고 열려 있는 아무 워크스페이스나, 자기가 돌고 있는 워크스페이스
  까지 대상이 됩니다(자기 자신은 아카이브만 — 삭제는 이 대화까지 함께 가져갑니다). 어느 권한
  모드에서든 어느 워크스페이스인지와 무엇을 잃는지를 적은 카드로 한 건씩 승인합니다.
- **울타리** — 대부분의 도구는 호출한 워크스페이스에만 작용합니다. `workspaceId` 를 받는 도구는
  호출자가 **직접 만든** 워크스페이스만 겨냥할 수 있고, `notify_child` 는 직계 스택 자식으로
  제한됩니다. 읽기 전용 도구는 승인 없이 돌고, 상태를 바꾸는 도구는 워크스페이스의 권한 모드를
  따라 승인 카드를 띄웁니다.
...
- **입력 보존·턴 중간 steering** — 작성 중 메시지는 workspace 전환에도 유지되고, 턴이 도는 중에
  보낸 후속 메시지는 턴이 끝나기를 기다리지 않고 곧바로 에이전트에 닿습니다.
