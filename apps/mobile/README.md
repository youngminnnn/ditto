...
컴포저의 클립 버튼으로 사진(보관함·카메라)과 문서를 붙일 수 있다. 바이트가 릴레이를 건너가는
길은 데스크톱과 다르다 — `commands.payload_ct` 가 **64KiB 하드 제약**이라 사진 한 장이 명령
하나에 들어가지 않기 때문이다. 그래서 파일을 조각내 `remote:upload` 여러 건으로 올리고,
`chat:send` 가 그 묶음을 `uploadId` 로 가리킨다. 계약의 단일 출처는 `src/shared/remote.ts`
의 「첨부」 절이고, 랩탑 쪽 조립은 `src/main/remote/uploads.ts` 다.

| | 값 | 왜 |
| --- | --- | --- |
| 첨부 1개 | 512 KB | 넘는 사진은 폰이 1568px JPEG 로 줄여서 맞춘다. 문서는 줄일 수 없으므로 거절한다 |
| 한 번에 | 5개 / 768 KB | 768KB ≈ 18조각. 기기당 **분당 30건** 명령 제한에서 watch 와 chat:send 를 빼고 남는 만큼이다 |
| 조각 1개 | 45,000 B | base64 로 60,000자 — 64KiB 암호문 상한 안에 봉투까지 넉넉히 들어간다 |

이미지는 데스크톱의 붙여넣기와 같이 base64 로 모델에 실린다. **문서는 랩탑의
`~/wooi/uploads/<workspaceId>/` 에 떨어지고 프롬프트 끝에 `@경로` 로 붙는다** — 데스크톱이
이미지 아닌 드롭 파일을 다루는 방식과 같은 규칙이다. 워크트리에 쓰지 않는 이유는 그쪽에
쓰면 `git status` 가 더러워지고 실수로 커밋되기 때문이다.

받는 파일 종류는 `REMOTE_FILE_EXTENSIONS` 가 정한다. 그 목록이 곧 **랩탑 디스크에 쓰일 수
있는 파일의 전부**라 실행 가능한 확장자는 넣지 않는다.

랩탑이 첨부를 모르는 옛 버전이면 전송이 실패한다. 폰은 첨부가 없을 때 인자를 두 개만 보내므로
첨부를 쓰지 않는 대화는 옛 랩탑에서도 그대로 된다.
...
그 대신 알고 있어야 하는 것 하나 — **Play 의 "12명 × 14일 연속" 요건은 비공개 테스트(closed)
트랙에서만 쌓인다. 내부 테스트(internal)는 하루도 카운트되지 않는다.** 태그를 밀어 자동
제출이 돌아도 내부 트랙만 채워지므로, 프로덕션 액세스를 신청하려면 그 빌드를 콘솔에서
비공개 트랙으로 올려야 시계가 돈다. 올릴 때 다시 굽지 않는다 — 비공개 트랙의
**"라이브러리에서 추가"** 로 이미 올라간 같은 번들을 고른다(versionCode 는 패키지당 전역이라
한 번 쓴 번호는 영구히 다시 못 쓴다).
