 ...
 */
/**
 * 이미 세워 둔 아티팩트 세션들. 게스트 가드가 "이 webContents 가 아티팩트인가" 를 물을 때 쓴다.
 *
 * `contents.session` 은 자기 파티션 이름을 알려주지 않으므로 이름으로 되물을 수가 없다.
 * 그래서 우리가 만든 세션을 기억해 두고 신원을 그 집합으로 판정한다.
 */
...
/**
 * 이 파티션의 아티팩트 세션과 그 울타리를 세운다 — **워크스페이스마다 하나**라서 게으르게,
 * 그리고 몇 번을 불러도 한 번만 선다.
 *
 * 부르는 자리가 `will-attach-webview` 인 것이 요점이다([[main/preview]]). 게스트가 붙기
 * **전에** protocol 핸들러와 거절 규칙이 서 있어야 하는데, 어느 워크스페이스의 세션이
 * 필요한지는 그 순간에야 알 수 있다(렌더러가 태그에 적어 온다).
 */
export function ensureArtifactSession(partition: string): Session {
...
/** 파티션 이름이 아티팩트 것일 때만 세운다(가드가 그대로 넘겨 부른다). */
export function ensureArtifactSessionFor(partition: string): boolean {
  if (!isArtifactPartition(partition)) return false
  ensureArtifactSession(partition)
  return true
}

/** 렌더러로 신호를 보내는 통로를 받아 둔다(앱 기동 시 1회). */
export function initArtifactDispatch(dispatch: (channel: string, payload: unknown) => void): void {
