...
/**
 * 사용량 제한 판정은 리뷰 쪽과 나눠 쓴다([[rateLimitText]]). 여기서 다시 내보내는 것은 이 모듈을
 * 통해 쓰던 곳(테스트 포함)이 그대로 돌게 하기 위함이다.
 */
export { RATE_LIMIT_ERROR, rateLimitResetAt }
    ...
    try {
      ...
      this.q = query({
        ...
        options: {
          ...
          // 연다([[types]] nativePeerInbound 가 저장된 accept 만 인정하는 이유도 같다).
          //
