 ...
 *
 * 설치가 실패해 프로세스가 살아남으면(읽기 전용 볼륨·서명 오류처럼 quitAndInstall 이 조용히
 * 아무것도 안 하는 경우) 표식이 켜진 채 굳는다. 그대로 두면 다음 진짜 종료도
 * `reasonForQuit()` 이 'update' 로 잘못 분류해 이어가기 사유가 어긋난다. 그래서 되돌린다.
 */
