          ...
          {FILTERS.map((f) => (
            <button
              ...
              className={
                ...
                (filter === f.key
                  ? 'bg-[#1b1f27] border-[#384050] text-neutral-100'
                  : 'bg-transparent border-[#23262d] text-neutral-400 hover:border-[#384050]')
              }
            >
              {f.label}
...
function OverviewCard({
  ...
  return (
    <button
      onClick={onOpen}
      className={
        'text-left rounded-xl border bg-[#0e1014] p-3 hover:border-[#384050] transition-colors ' +
        (flags.attention
          ? 'border-amber-500/40'
          : flags.running
            ? 'border-blue-500/30'
