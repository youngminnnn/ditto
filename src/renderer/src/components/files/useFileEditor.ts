 ...
 * 파일 뷰어의 인라인 편집 상태.
 *
 * 파일 탭과 All files 패널이 **같은 저장 규칙**을 써야 해서 훅으로 뺐다. 특히 충돌
 * 처리는 한쪽에만 있으면 없는 쪽이 남의 작업을 덮어쓰는 길이 된다.
 *
 * 초안은 **경로별로** 들고 있다. 파일 탭은 여러 개를 동시에 열어 두고 오가는 물건이라, 고치던
 * 중에 다른 파일 탭을 잠깐 봤다 오는 일이 자연스럽다 — 그때마다 초안을 버리면 사용자는 탭
 * 사이를 옮겨 다니기를 두려워하게 된다.
 *
 * 주의: 이 훅 자체는 "닫을 때 확인" 을 갖고 있지 않다(예전 오버레이의 closeWithGuard 는 탭
 * 전환으로 옮기면서 빠졌다) — 탭을 닫는 경로(TabStrip 의 X, ⌘W)가 `useFileEditor` 를 들고
 * 있는 FileTab 바깥에 있어서, 지금은 저장하지 않은 채로 파일 탭을 닫으면 그 초안이 조용히
 * 사라진다. All files 패널(FileBrowser)은 여전히 자기 안에서 "목록으로" 눌렀을 때만 확인을
 * 받는다 — 그쪽은 손실 지점이 하나뿐이라 가능했다.
 *
