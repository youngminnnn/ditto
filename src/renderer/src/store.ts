...
interface UIState {
  ...
  enqueueMessage: (workspaceId: string, text: string) => void
...
export const useStore = create<UIState>((set, get) => ({
      ...
      if (event.type === 'item') {
        ...
        if (event.type === 'status' && event.status === 'idle') {
          ...
          if (queued && queued.length) {
            ...
            for (const text of queued) void window.api.chat.send(workspaceId, text)
          }
        }
  ...
  enqueueMessage: (workspaceId, text) =>
    set((s) => ({
      messageQueue: { ...s.messageQueue, [workspaceId]: [...(s.messageQueue[workspaceId] ?? []), text] }
    })),
