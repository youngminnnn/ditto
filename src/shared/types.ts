...
/**
 * 아티팩트 `<webview>` 세션 파티션의 접두사. 실제 이름은 [[artifactPartition]] 이 만든다.
 *
 ...
 *
 * `persist:` 를 **일부러 뺐다** — 이 세션들은 디스크에 쿠키도 스토리지도 남기지 않는다.
 * 남길 이유가 없고(아티팩트에 로그인할 곳이 없다), 남기면 그게 곧 적대적 콘텐츠가
 * 앱 재시작을 건너뛰고 상태를 잇는 통로가 된다.
 */
export const ARTIFACT_PARTITION_PREFIX = 'wooi-artifact-'

/**
 * 이 워크스페이스의 아티팩트 파티션.
 *
 * **워크스페이스마다 하나다.** 하나를 공유하면 비영속이라 디스크에는 안 남아도 앱이 사는
 * 동안 localStorage·sessionStorage 가 워크스페이스 경계를 넘어 섞인다 — 저장소 A 를 읽고
 * 만든 아티팩트가 저장소 B 의 아티팩트가 남긴 것을 읽을 수 있다는 뜻이다. 격리가 이 기능의
 * 값이므로 그 틈을 열어 두지 않는다.
 */
export function artifactPartition(workspaceId: string): string {
  return `${ARTIFACT_PARTITION_PREFIX}${workspaceId}`
}

/** 이 파티션 이름이 아티팩트 것인가(게스트 가드가 allowlist 로 쓴다). */
export function isArtifactPartition(partition: string): boolean {
  return partition.startsWith(ARTIFACT_PARTITION_PREFIX)
}
